...
## :exclamation: 文档同步规则 — 必须遵守

> **:warning: 修改代码前必须阅读相关实现文档，修改后必须同步更新文档。违反此规则将导致文档与代码不一致。**

`docs/implementation/` 目录包含各功能点的详细实现文档。

### :one: 修改前 — 必须先看文档
必须先查看 `docs/implementation/README.md` 目录索引，找到对应的功能点实现文档，理解现有实现，再开始修改代码。

### :two: 修改后 — 必须同步更新文档
必须在同一 PR/commit 中同步更新或新增功能点实现文档。

### :three: 保持一致性
文档中的文件路径、类名、函数名、设计描述必须与实际代码匹配，不得有过时或错误的引用。
