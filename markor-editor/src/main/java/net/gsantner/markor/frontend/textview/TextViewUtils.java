...
public final class TextViewUtils {
    ...
    public static String interpolateSnippet(String text, final CharSequence title, final CharSequence selectedText) {
        ...
        final String time = new SimpleDateFormat("HH:mm", Locale.getDefault()).format(new Date(current));
        final String date = new SimpleDateFormat("yyyy-MM-dd", Locale.getDefault()).format(new Date(current));
        final String weekday = new SimpleDateFormat("EEEE", Locale.getDefault()).format(new Date(current));
    ...
    public static String interpolateEscapedDateTime(final String text) {
        ...
        for (int i = 0; i < text.length(); i++) {
            ...
            if (c == '\\' && !isEscaped) {
                ...
                inDate = false;
                interpolated.append(new SimpleDateFormat(temp.toString(), Locale.getDefault()).format(new Date(System.currentTimeMillis())));
